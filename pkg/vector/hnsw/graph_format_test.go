...
func TestGraphFormatRoundTripExactTopologyAndSearch(t *testing.T) {
	...
	opened, openedMetadata, err := OpenGraph(bytes.NewReader(first), original, reference, DefaultGraphLimits())
	if err != nil {
		t.Fatal(err)
	}
...
func TestGraphFormatEmptySingletonAndCosine(t *testing.T) {
	...
	for _, test := range spaces {
		t.Run(test.name, func(t *testing.T) {
			...
			opened, gotMetadata, err := OpenGraph(bytes.NewReader(data), original, reference, DefaultGraphLimits())
			if err != nil {
				t.Fatal(err)
			}
...
func TestOpenGraphValidatesPreparedVectorSource(t *testing.T) {
	original := newReaderForTest(t, readerTestSpace(t, 2, vector.MetricL2Squared), readerTestGraph())
	reference := testVectorFileReference()
	...
	for _, mismatch := range []*graphPreparedSource{
		...
		if _, _, err := OpenGraph(bytes.NewReader(data), mismatch, reference, DefaultGraphLimits()); !errors.Is(err, ErrGraphVectorSource) || mismatch.reads != 0 {
	...
	if _, _, err := OpenGraph(bytes.NewReader(data), partial, reference, DefaultGraphLimits()); !errors.Is(err, ErrGraphVectorSource) {
...
func TestGraphFormatRejectsReferenceSizeAndIntegrityFailures(t *testing.T) {
	...
	wrong := reference
	wrong.Size++
	if _, _, err := OpenGraph(bytes.NewReader(data), original, wrong, DefaultGraphLimits()); !errors.Is(err, ErrVectorFileRefMismatch) {
	...
	for _, size := range []int{0, 3, graphFormatHeaderSize - 1, len(data) - 1} {
		if _, _, err := OpenGraph(bytes.NewReader(data[:size]), original, reference, DefaultGraphLimits()); err == nil {
			t.Fatalf("truncated graph size %d was accepted", size)
		}
	}
	trailing := append(append([]byte(nil), data...), 0)
	if _, _, err := OpenGraph(bytes.NewReader(trailing), original, reference, DefaultGraphLimits()); !errors.Is(err, ErrCorruptGraphData) {
		t.Fatalf("trailing graph error = %v", err)
	}
	corrupt := append([]byte(nil), data...)
	corrupt[graphFormatHeaderSize] ^= 1
	if _, _, err := OpenGraph(bytes.NewReader(corrupt), original, reference, DefaultGraphLimits()); !errors.Is(err, ErrCorruptGraphData) {
	...
	if _, _, err := OpenGraph(bytes.NewReader(unsupported), original, reference, DefaultGraphLimits()); !errors.Is(err, ErrUnsupportedGraphVersion) {
...
func TestGraphFormatRejectsMalformedPackedTopologyAndCanonicalBytes(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if _, _, err := OpenGraph(bytes.NewReader(data), original, reference, DefaultGraphLimits()); !errors.Is(err, ErrCorruptGraphData) {
	...
	opened, _, err := OpenGraph(bytes.NewReader(futureBuild), original, reference, DefaultGraphLimits())
	...
	if _, _, err := OpenGraph(bytes.NewReader(padded), singleton, reference, DefaultGraphLimits()); !errors.Is(err, ErrCorruptGraphData) {
...
func TestOpenGraphContextCancellation(t *testing.T) {
	...
	cancel()
	if _, _, err := OpenGraphContext(ctx, bytes.NewReader(data), original, reference, DefaultGraphLimits()); !errors.Is(err, context.Canceled) {
	...
	if _, _, err := OpenGraphContext(ctx, bytes.NewReader(data), source, reference, DefaultGraphLimits()); !errors.Is(err, context.Canceled) {
...
func TestOpenGraphEnforcesConfiguredLimits(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			test.change(&limits)
			if _, _, err := OpenGraph(bytes.NewReader(data), original, reference, limits); !errors.Is(err, ErrGraphLimitExceeded) {
				t.Fatalf("error = %v", err)
			}
		})
	}
...
func TestOpenedReaderConcurrentSearch(t *testing.T) {
	...
	opened, _, err := OpenGraph(bytes.NewReader(data), original, reference, DefaultGraphLimits())
	if err != nil {
		t.Fatal(err)
	}
...
func TestGraphFormatSourceRejectsNonCanonicalNegativeZero(t *testing.T) {
	...
	if _, _, err := OpenGraph(bytes.NewReader(data), source, testVectorFileReference(), DefaultGraphLimits()); !errors.Is(err, ErrGraphVectorSource) {
